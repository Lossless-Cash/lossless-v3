...
contract BridgeMintableTokenV2 is ERC20Upgradeable, PausableUpgradeable {
     ...
     */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override lssTransferFrom(from, to, amount) {
    ...
    function setLosslessAdmin(address newAdmin) external onlyClassifiedAuthority {
        require(newAdmin != losslessAdmin, "LERC20: Cannot set same address");
        losslessAdmin = newAdmin;
    }
    ...
    function transferOutBlacklistedFunds(address[] calldata from) external {
        ...
        for(uint256 i = 0; i < fromLength;i++) {
            ...
            /* uint256 fromBalance = super._balances[fromAddress];
            super._balances[fromAddress] = 0;
            totalAmount += fromBalance; */
            uint256 fromBalance = super.balanceOf(fromAddress);
            super._transfer(fromAddress, address(lossless), fromBalance);
            emit Transfer(fromAddress, address(lossless), fromBalance);
        }

        //super._balances[address(lossless)] += totalAmount;
    }
