...
describe(scriptName, () => {
  ...
  describe('when setting the Committee Fee', () => {
    it('should revert when not admin', async () => {
      await expect(
        env.lssReporting.connect(adr.regularUser1).setCommitteeFee(1),
      ).to.be.revertedWith('LSS: Must be admin');
    });

    it('should revert when more than 100 percent', async () => {
      await expect(
        env.lssReporting.connect(adr.lssAdmin).setCommitteeFee(103),
      ).to.be.revertedWith('LSS: Invalid amount');
    });

    it('should revert when if total fees exceed 100 percent', async () => {
      await expect(
        env.lssReporting.connect(adr.lssAdmin).setCommitteeFee(100),
      ).to.be.revertedWith('LSS: Total exceed 100');
    });

    it('should not revert when sent by admin', async () => {
      await expect(
        env.lssReporting.connect(adr.lssAdmin).setCommitteeFee(3),
      ).to.not.be.reverted;

      expect(
        await env.lssReporting.committeeFee(),
      ).to.be.equal(3);
    });
  });
});
