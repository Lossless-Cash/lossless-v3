    ...
    beforeEach(async () => {
        ...
        describe('when the staking period is active', ()=>{
          beforeEach(async ()=>{
            ...
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker1.address, env.stakeAmount);
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker2.address, env.stakeAmount);
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker3.address, env.stakeAmount);
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker4.address, env.stakeAmount);
        ...
        describe('when the staking period is inactive', ()=>{
          beforeEach(async ()=>{
            ...
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.reporter1.address, env.stakeAmount*2);
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker1.address, env.stakeAmount+env.stakeAmount);
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker2.address, env.stakeAmount*2);
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker3.address, env.stakeAmount*2);
            await env.lssToken.connect(adr.lssInitialHolder).transfer(adr.staker4.address, env.stakeAmount*2);
    
            await env.lssToken.connect(adr.reporter1).approve(env.lssReporting.address, env.stakeAmount*2);
            ...
            await lssGovernance.connect(adr.lssAdmin).addCommitteeMembers([adr.member1.address, adr.member2.address, adr.member3.address], 2);
            
            await lssGovernance.connect(adr.lssAdmin).losslessVote(1, true);
            await lssGovernance.connect(adr.lerc20Admin).tokenOwnersVote(1, true);
            await lssGovernance.connect(adr.member1).committeeMemberVote(1, true);
            await lssGovernance.connect(adr.member2).committeeMemberVote(1, true);
  
            await lssGovernance.connect(adr.lssAdmin).resolveReport(1);
