...
describe(scriptName, () => {
  ...
  describe('when the report has been solved negatively', () => {
    ...
    describe('when revshare admin claims', () => {
      it('should revert', async () => {
        await expect(
          env.lssGovernance.connect(adr.regularUser5).revshareClaim(1),
        ).to.be.revertedWith('LSS: Report solved negatively');
  ...
  describe('when the report has been solved correctly', () => {
    ...
    describe('when revshare admin claims', () => {
      it('should not revert', async () => {
        ...
        let balance;
        expect(
          (balance = await lerc20Token.balanceOf(adr.regularUser5.address)),
        ).to.be.equal(0);

        await env.lssGovernance.connect(adr.regularUser5).revshareClaim(1);

        expect(await lerc20Token.balanceOf(adr.regularUser5.address)).to.be.equal(
          (reportedAmount * losslessReward) * revsharePercent,
        );
      });
      ...
      it('should emit event', async () => {
        await expect(
          env.lssGovernance.connect(adr.regularUser5).revshareClaim(1),
        ).to.emit(env.lssGovernance, 'RevshareClaim').withArgs(
          lerc20Token.address,
          1,
          (reportedAmount * losslessReward) * revsharePercent,
        );
    ...
    describe('when revshare admin claim is called other than lossless admin', () => {
      it('should revert', async () => {
        await expect(
          env.lssGovernance.connect(adr.reporter1).revshareClaim(1),
        ).to.be.revertedWith('LSS: Must be revshareAdmin');
      });
    });
  });
});
