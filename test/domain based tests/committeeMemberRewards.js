...
describe(scriptName, () => {
  ...
  describe('when members claim their rewards', () => {
    ...
    describe('when resolved positively', () => {
      describe('when committee members participate', () => {
        beforeEach(async () => {
        ...
        it('should not revert when member 1 claims', async () => {
          ...
          expect(await lerc20Token.balanceOf(adr.member1.address)).to.be.equal(
        ...
        it('should emit event when member claims', async () => {
          await expect(
            ...
            .to.emit(env.lssGovernance, 'CommitteeMemberClaim')
            .withArgs(
              1,
              adr.member1.address,
              (reportedAmount * committeeReward) / 4,
        ...
        it('should not revert when member 2 claims', async () => {
          ...
          expect(await lerc20Token.balanceOf(adr.member2.address)).to.be.equal(
            (reportedAmount * committeeReward) / 4,
          );
        });
        it('should not revert when member 3 claims', async () => {
          ...
          expect(await lerc20Token.balanceOf(adr.member3.address)).to.be.equal(
            (reportedAmount * committeeReward) / 4,
          );
        });
        it('should not revert when member 4 claims', async () => {
          ...
          expect(await lerc20Token.balanceOf(adr.member4.address)).to.be.equal(
            (reportedAmount * committeeReward) / 4,
          );
        });
      });
      describe('when committee members dont participate', () => {
        beforeEach(async () => {
          await env.lssGovernance.connect(adr.lssAdmin).losslessVote(1, true);
          await env.lssGovernance
            .connect(adr.lerc20Admin)
        ...
        it('should not revert if lossless claims amount', async () => {
          await expect(
            env.lssGovernance
              .connect(adr.lssAdmin)
              .losslessAdminClaimCommitteeTokens(1),
          ).to.not.be.reverted;
        });
      });
    });
