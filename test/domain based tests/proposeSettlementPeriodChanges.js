    ...
    describe('when proposing a new settlement period', () => {
      ...
      it('should emit event', async () => {
        await expect(
          ...
          .to.emit(env.lssController, 'NewSettlementPeriodProposal')
          .withArgs(lerc20Token.address, 5 * 60);
      });
    ...
    describe('when the settlement period is active', () => {
      ...
      describe('when transfering to a dex', () => {
        beforeEach(async () => {
          ...
          await lerc20Token
            .connect(adr.lerc20InitialHolder)
            .transfer(adr.regularUser1.address, 50);
        });
        ...
        it('should revert when transfering unsettled over the dex threshold', async () => {
          await expect(
            lerc20Token
              .connect(adr.regularUser1)
              .transfer(adr.dexAddress.address, 145),
        ...
        it('should not revert when transfering unsettled below the dex threshold', async () => {
          await expect(
            lerc20Token
              .connect(adr.regularUser1)
              .transfer(adr.dexAddress.address, 119),
          ).to.not.be.reverted;
        });
        ...
        it('should not revert when transfering settled tokens', async () => {
          await expect(
            lerc20Token
              .connect(adr.regularUser1)
              .transfer(adr.dexAddress.address, 99),
          ).to.not.be.reverted;
        });
      });
      ...
      describe('when transfering between users', () => {
        beforeEach(async () => {
          ...
          await lerc20Token
            .connect(adr.lerc20InitialHolder)
            .transfer(adr.regularUser2.address, 100);
        ...
        it("should not revert if 5 minutes haven't passed but its the first transfer over the unsettled amount", async () => {
          await expect(
            lerc20Token
              .connect(adr.regularUser2)
              .transfer(adr.regularUser4.address, 55),
          ).to.not.be.reverted;
        });
        ...
        it('should not revert if 5 minutes have passed on first transfer', async () => {
          ...
          await expect(
            lerc20Token
              .connect(adr.regularUser1)
              .transfer(adr.regularUser3.address, 5),
          ).to.not.be.reverted;
        ...
        describe('when transfering at the same timestamp', () => {
          ...
          it('should not revert', async () => {
            ...
            await lerc20Token
              .connect(adr.regularUser2)
              .approve(mockTransfer.address, 200);
            ...
            await expect(
              mockTransfer.testSameTimestamp(
      ...
      describe('when transfering between users with transferFrom', () => {
        beforeEach(async () => {
          ...
          await lerc20Token
            .connect(adr.regularUser1)
            .approve(adr.regularUser3.address, 50);
          await lerc20Token
            .connect(adr.regularUser2)
            .approve(adr.regularUser3.address, 50);
        });
      ...
      describe('when emergency mode is active', () => {
        ...
        describe('when transfering settled tokens', () => {
          it('should not revert', async () => {
            ...
            await expect(
              lerc20Token
                .connect(adr.regularUser1)
                .transfer(adr.regularUser3.address, env.stakingAmount),
            ).to.not.be.reverted;
          });
        });
        ...
        it('users are able to transfer settled LERC20 tokens when a report is filed', async function () {
      ...
      describe('when transfering to a dex', () => {
        ...
        it('should not revert when transfering unsettled below the dex threshold', async () => {
          await expect(
            lerc20Token
              .connect(adr.regularUser1)
              .transfer(adr.dexAddress.address, 115),
          ).to.not.be.reverted;
        });
