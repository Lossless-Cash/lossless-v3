...
describe(scriptName, () => {
  beforeEach(async () => {
    ...
    await lerc20Token
      ...
      .initialize('Chainport LERC20', 'CLERC', 18, adr.regularUser1.address);

    await lerc20Token
      .connect(adr.lerc20Admin)
      .setLosslessController(env.lssController.address);
    await lerc20Token
      .connect(adr.lerc20Admin)
      .setLosslessAdmin(adr.lerc20Admin.address);
    await lerc20Token
      .connect(adr.lerc20Admin)
  ...
  describe('when transfering among users', () => {
    ...
    describe('when Lossless is turned off', () => {
      beforeEach(async () => {
        await anotherLerc20Token
          ...
          .transfer(adr.regularUser3.address, 1000);
        await anotherLerc20Token
          .connect(adr.regularUser1)
          .transfer(adr.regularUser4.address, 1000);
      });
      it('should allow transfering all received tokens', async () => {
        await expect(
          anotherLerc20Token
            .connect(adr.regularUser3)
            .transfer(adr.regularUser4.address, 1000),
        ).to.not.be.reverted;
        await expect(
          anotherLerc20Token
            .connect(adr.regularUser4)
            .transfer(adr.regularUser5.address, 2000),
        ).to.not.be.reverted;
      });
      it('should allow multiple transactions without restrictions', async () => {
        await expect(
          anotherLerc20Token
            .connect(adr.regularUser3)
            .transfer(adr.regularUser5.address, 500),
        ).to.not.be.reverted;
        await expect(
          anotherLerc20Token
            .connect(adr.regularUser3)
        ...
        await expect(
          anotherLerc20Token
            .connect(adr.regularUser4)
            .transfer(adr.regularUser5.address, 400),
        ).to.not.be.reverted;
        await expect(
          anotherLerc20Token
            .connect(adr.regularUser4)
            .transfer(adr.regularUser5.address, 600),
        ).to.not.be.reverted;
        await expect(
          anotherLerc20Token
            .connect(adr.regularUser5)
            .transfer(adr.regularUser1.address, 2000),
        ).to.not.be.reverted;
      });
    });
    ...
    describe('when Lossless is turned on', () => {
      beforeEach(async () => {
        await env.lssController
          .connect(adr.lerc20Admin)
          .proposeNewSettlementPeriod(lerc20Token.address, 600);
        await ethers.provider.send('evm_increaseTime', [
      ...
      describe('when the settlement period is 10 minutes', () => {
        ...
        it('should allow transfering unsettled tokens once per period', async () => {
          ...
          await expect(
            lerc20Token
              .connect(adr.regularUser5)
              .transfer(adr.regularUser1.address, 500),
          ).to.not.be.reverted;
        });
        ...
        it('should revert transfering unsettled tokens more than once per period with large locks queue', async () => {
          for (let i = 0; i < 150; i++) {
            await lerc20Token
              .connect(adr.lerc20InitialHolder)
              .transfer(adr.regularUser5.address, 1);
          }

          await expect(
            lerc20Token
              .connect(adr.regularUser5)
              .transfer(adr.regularUser4.address, 101),
          ).to.not.be.reverted;

          await expect(
            lerc20Token
              .connect(adr.regularUser5)
              .transfer(adr.regularUser4.address, 5),
        ...
        it('should revert transfering unsettled tokens more than once per period', async () => {
          ...
          await expect(
            lerc20Token
              .connect(adr.regularUser5)
              .transfer(adr.regularUser1.address, 250),
          ).to.not.be.reverted;
        ...
        it('should allow transfering settled and some unsettled tokens once', async () => {
          ...
          await expect(
            lerc20Token
              .connect(adr.regularUser5)
              .transfer(adr.regularUser1.address, 750),
          ).to.not.be.reverted;
          await expect(
            lerc20Token
              .connect(adr.regularUser5)
      ...
      describe('when all checkpoints are expired', () => {
        it('should allow transfering all settled tokens', async () => {
          for (let i = 0; i < 20; i++) {
            await lerc20Token
              .connect(adr.lerc20InitialHolder)
              .transfer(adr.regularUser3.address, 1);
            await ethers.provider.send('evm_increaseTime', [
              Number(time.duration.minutes(10)),
            ]);
          }
          await expect(
            lerc20Token
              .connect(adr.regularUser3)
              .transfer(adr.regularUser5.address, 20),
          ).to.not.be.reverted;
        });
      });
      ...
      describe('when all checkpoints are active', () => {
        ...
        it('should allow transfering unsettled tokens once per period', async () => {
          await expect(
            lerc20Token
              .connect(adr.regularUser3)
              .transfer(adr.regularUser5.address, 10),
          ).to.not.be.reverted;
        });
        it('should revert transfering unsettled tokens more than once per period', async () => {
          await expect(
      ...
      describe('when only one checkpoint is active', () => {
        ...
        it('should allow transfering unsettled tokens once per period', async () => {
          await lerc20Token
            .connect(adr.lerc20InitialHolder)
            .transfer(adr.regularUser3.address, 2);
          await expect(
            lerc20Token
              .connect(adr.regularUser3)
              .transfer(adr.regularUser5.address, 1),
        ...
        it('should allow transfering settled tokens', async () => {
          await expect(
            lerc20Token
              .connect(adr.regularUser3)
              .transfer(adr.regularUser5.address, 19),
          ).to.not.be.reverted;
        });
      });
      ...
      describe('when only one checkpoint is inactive', () => {
        ...
        it('should revert transfering unsettled tokens more than once per period', async () => {
          // used up 1 from settled
          // and 18 from unsettled
          await expect(
            lerc20Token
              .connect(adr.regularUser3)
              .transfer(adr.regularUser5.address, 3),
          ).to.not.be.reverted;
  ...
  describe('when transfering to a dex', () => {
    ...
    it('should allow transfering settled tokens', async () => {
      ...
      await lerc20Token
        .connect(adr.lerc20InitialHolder)
        .transfer(adr.regularUser3.address, 500);

      await expect(
        lerc20Token
          .connect(adr.regularUser3)
          .transfer(adr.dexAddress.address, 1000),
      ).to.not.be.reverted;
    });
    it('should allow transfering unsettled below threshold', async () => {
      ...
      await expect(
        lerc20Token
          .connect(adr.regularUser3)
          .transfer(adr.dexAddress.address, 1020),
      ).to.not.be.reverted;
    });
    it('should revert transfering unsettled above threshold', async () => {
      ...
      await lerc20Token
        .connect(adr.lerc20InitialHolder)
        .transfer(adr.regularUser3.address, 520);

      await expect(
        lerc20Token
          .connect(adr.regularUser3)
          .transfer(adr.dexAddress.address, 1021),
